...
# --- Configuration ---
TOKEN = os.getenv('BOT_TOKEN')  # گرفتن BOT_TOKEN from environment variable
if not TOKEN:
...
# --- Database Setup ---
# =====================================================
# 🔥 AUTO BACK SYSTEM (AUTO RESTORE HOSTED BOTS)
# =====================================================
def auto_run_all_hosted_scripts():
    logger.info("🔄 Auto restoring all hosted scripts after restart...")
    ...
    for user_id, files in user_files.items():
        ...
        for file_name, file_type in files:
            ...
            if not os.path.exists(script_path):
                logger.warning(f"Missing file: {script_path}")
                continue
            ...
            try:
                logger.info(f"🚀 Auto starting {file_name} for user {user_id}")

                if file_type == "py":
                    threading.Thread(
                        target=run_script,
                        args=(script_path, user_id, user_folder, file_name, fake_msg)
                    ).start()
                ...
                logger.error(f"Auto start failed: {file_name} -> {e}")
...
time.sleep(5)
threading.Thread(
    target=auto_run_all_hosted_scripts,
    daemon=True
).start()
